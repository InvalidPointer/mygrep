...
{
    if (argc != 3) {
        cout << "Wrong params! Usage: " << argv[0] << " %path_to_tests% {match|search|syntax}" << endl;
        return 0;
    }

    mode mode;
    if (!string(argv[2]).compare("match")) {
        mode = MATCH;
    } else if (!string(argv[2]).compare("search")) {
        mode = SEARCH;
    } else {
        mode = SYNTAX;
    }
    ...
    for (auto it = test_fnames.begin(); it < test_fnames.end(); it++) {
        ...
        if (mode == SYNTAX) {
            int line = 0;
            while (expr_file >> expr) {
                string res;
                res_file >> res;

                try {
                    Regexp r(expr);
                    if (res.compare("Correct")) {
